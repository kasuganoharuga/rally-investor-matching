...
const CSV_HEADERS = [
  ...
  "Planning Cheque",
  "Capacity Currency",
  "Planning Conversion Factor (%)",
  "Risk-adjusted Contribution",
  "Cheque Estimate Source",
  "Round Size Minimum",
  "Round Size Maximum",
  "Round Size Currency",
...
function matchRow(
  ...
  return [
    ...
    match.confidence,
    match.capacity_estimate?.role.replaceAll("_", " "),
    match.capacity_estimate?.planning_cheque,
    match.capacity_estimate?.currency,
    match.capacity_estimate
      ? Math.round(match.capacity_estimate.conversion_factor * 1000) / 10
      : "",
    match.capacity_estimate?.risk_adjusted_amount,
    match.capacity_estimate?.cheque_source.replaceAll("_", " "),
    cheque.minimum,
    cheque.maximum,
    cheque.currencies,
