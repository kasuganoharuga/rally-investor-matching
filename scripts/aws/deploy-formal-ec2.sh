...
Before=rally-api.service rally-web.service

[Service]
Type=oneshot
ExecStart=/bin/bash /opt/rally-current/scripts/aws/refresh-public-urls.sh
RemainAfterExit=yes
...
After=network-online.target rally-refresh-public-urls.service
Wants=network-online.target
Requires=rally-refresh-public-urls.service
...
systemctl daemon-reload
systemctl enable rally-refresh-public-urls.service
